...
export default class LinkCursor extends Command {
    static description = 'Link your Cursor IDE to PromptBrain';
    ...
    async run(): Promise<void> {
        ...
        try {
            ...
            logger.info(chalk.blue('🔗 Starting Cursor authorization...'));

            // Start the OAuth flow
            const { auth_url } = await integrationApi.startLink('cursor', cliSession);
            ...
            await poll(async () => {
                try {
                    ...
                    const cursor = integrations.find(i => i.provider === 'cursor');

                    if (cursor && cursor.connected) {
                        isLinked = true;
                        return true;
                    }
                    return false;
            ...
            if (isLinked) {
                logger.success(chalk.green('✓ Cursor linked successfully!'));
                logger.info(chalk.dim('Your Cursor IDE is now connected to PromptBrain.'));
            } else {
                logger.error('Failed to link Cursor. Please try again.');
                process.exit(1);
            }
            ...
            if (error.message === 'Polling timeout exceeded') {
                ...
                logger.error(`Failed to link Cursor: ${error.response.data?.message || error.message}`);
            } else {
